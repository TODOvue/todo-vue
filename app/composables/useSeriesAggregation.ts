...
const getSeriesDateValue = (value: BlogPost['date']): number => new Date(value ?? 0).getTime()

export const useSeriesAggregation = (limit?: number) => {
  const { t } = useI18n()
  ...
  const seriesList = computed<SeriesItem[]>(() => {
    ...
    blogStore.blogPosts.value.forEach((post) => {
      ...
      const postDate = getSeriesDateValue(post.date)
